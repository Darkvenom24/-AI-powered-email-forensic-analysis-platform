...
def generate_pdf_report(result, output_path):
    ...
    ioc_rows = [
        ("URLs", ", ".join(str(x) for x in (iocs.get("urls") or [])) or "None detected"),
        ("IP Addresses", ", ".join(str(x) for x in (iocs.get("ips") or [])) or "None detected"),
        ("Email Addresses", ", ".join(str(x) for x in (iocs.get("emails") or [])) or "None detected"),
        ("Domains", ", ".join(str(x) for x in (iocs.get("domains") or [])) or "None detected"),
    ]
    ...
    story.append(
        Paragraph(
            "IOC extraction is automated and should be validated by an investigator before taking action.",
            styles["tiny"]
        )
    )
